...
module router_fsm_tb;
  ...
  //Task for checking FSM states with corresponding outputs for payload data length < 14
  task t1;
    begin
      @(negedge clock);
      pkt_valid = 1;
  ...
  task t2;
    begin
      ...
      repeat (2) @(negedge clock);
      fifo_full = 1;
      repeat (3) @(negedge clock) fifo_full = 0;
      @(negedge clock) parity_done = 1;
    end
  endtask

  //Task for checking FSM states with corresponding outputs for payload data length = 15
  task t3;
    begin
      @(negedge clock);
      pkt_valid = 1;
  ...
  task t4;
    begin
      ...
      @(negedge clock) parity_done = 0;
      low_pkt_valid = 0;
      repeat (3) @(negedge clock) pkt_valid = 0;
